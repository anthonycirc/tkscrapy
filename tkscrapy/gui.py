...
class App(tkinter.Tk):
    ...
    def call_subprocess_command(self):
        ...
        if self.url_field.get() and self.parent_selector.get():
            command = "scrapy", "runspider",  os.path.join(os.path.abspath(os.path.dirname(__file__)),"tkspider.py"), "-o", f"tkspider.{self.export_var.get()}", "-a", "url=" + self.url_field.get(), "-a", "parent_selector=" + self.parent_selector.get(), "-a", "selectors=" + self.selector_entry.get(
